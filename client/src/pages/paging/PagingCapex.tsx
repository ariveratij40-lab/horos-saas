...
import { DollarSign, Wrench, TrendingUp, Package } from "lucide-react";
import { PieChart, Pie, Cell, LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from "recharts";

const COLORS = ["#3b82f6", "#10b981", "#f59e0b", "#ef4444"];
const fmt = (n: number) => n.toLocaleString("es-MX", { style: "currency", currency: "MXN", maximumFractionDigits: 0 });

export default function PagingCapex() {
  ...
  const { data: powerSupplies = [] } = trpc.pagingPower.list.useQuery(undefined);

  const capexCategories = [
    { name: "Amplificadores", count: amplifiers.length, value: amplifiers.reduce((s: number, r: any) => s + (Number(r.amount) || 0), 0) },
    { name: "Bocinas", count: speakers.length, value: speakers.reduce((s: number, r: any) => s + (Number(r.amount) || 0), 0) },
    { name: "Consolas", count: consoles.length, value: consoles.reduce((s: number, r: any) => s + (Number(r.amount) || 0), 0) },
    { name: "Fuentes", count: powerSupplies.length, value: powerSupplies.reduce((s: number, r: any) => s + (Number(r.amount) || 0), 0) },
  ];
  ...
  const opexAnnual = Math.round(totalCapex * 0.07);
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <TrendingUp className="w-6 h-6 text-primary" />Análisis CAPEX / OPEX — Sistema de Voceo
        </h1>
        <p className="text-sm text-muted-foreground mt-1">Inversión de capital, costos operativos y proyección TCO del sistema de voceo</p>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { icon: <DollarSign className="w-5 h-5" />, label: "CAPEX Total", value: fmt(totalCapex), color: "text-primary" },
          { icon: <Wrench className="w-5 h-5" />, label: "OPEX Anual", value: fmt(opexAnnual), color: "text-amber-500" },
          ...
          <Card>
            ...
            <CardContent>
              <div className="mt-4 p-4 bg-muted/30 rounded-lg">
                ...
                <p className="text-xs text-muted-foreground mt-1">CAPEX: {fmt(totalCapex)} + OPEX acumulado: {fmt(opexAnnual * 5)}</p>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
