...
export default function PagingMaintenance() {
  ...
  const { data: consoles = [] } = trpc.pagingConsoles.list.useQuery(undefined);
  const { data: logs = [], refetch } = trpc.pagingMaintenance.list.useQuery(undefined);

  const createMut = trpc.pagingMaintenance.create.useMutation({ onSuccess: () => { toast.success("Registro creado"); setShowForm(false); refetch(); } });
  const updateMut = trpc.pagingMaintenance.update.useMutation({ onSuccess: () => { toast.success("Registro actualizado"); setShowForm(false); refetch(); } });
  const deleteMut = trpc.pagingMaintenance.delete.useMutation({ onSuccess: () => { toast.success("Registro eliminado"); setDeleteItem(null); refetch(); } });
  ...
  const allEquipment = [
    ...
    ...consoles.map((c: any) => ({ id: `con-${c.id}`, name: `${c.idConsole ?? "Consola"} — ${c.marca} ${c.modelo}` })),
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Wrench className="w-6 h-6 text-primary" />Mantenimiento — Sistema de Voceo
          </h1>
          <p className="text-sm text-muted-foreground mt-1">Historial y programas de mantenimiento del sistema de voceo</p>
        </div>
      ...
      <DeleteConfirmDialog
        ...
        itemName="este registro de mantenimiento"
        itemType="registro"
