...
import { Wrench, Plus, Search, Pencil, Trash2, RefreshCw, CheckCircle2, Clock, AlertTriangle } from "lucide-react";
...
export default function CablingMaintenance() {
  ...
  const { data: logs = [], refetch } = trpc.cabledMaintenance.list.useQuery(undefined);

  const createMut = trpc.cabledMaintenance.create.useMutation({ onSuccess: () => { toast.success("Registro creado"); setShowForm(false); refetch(); } });
  const updateMut = trpc.cabledMaintenance.update.useMutation({ onSuccess: () => { toast.success("Registro actualizado"); setShowForm(false); refetch(); } });
  const deleteMut = trpc.cabledMaintenance.delete.useMutation({ onSuccess: () => { toast.success("Registro eliminado"); setDeleteItem(null); refetch(); } });
  ...
  const allEquipment = [
    ...
    ...outlets.map((o: any) => ({ id: `ro-${o.id}`, name: `${o.idOutlet ?? "Roseta"} — ${o.nombre}` })),
  ];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Wrench className="w-6 h-6 text-primary" />Mantenimiento — Cableado Estructurado
          </h1>
          <p className="text-sm text-muted-foreground mt-1">Historial y programas de mantenimiento del sistema de cableado estructurado</p>
        </div>
      ...
      <div className="rounded-lg border overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {logs.length === 0 && <tr><td colSpan={7} className="px-3 py-8 text-center text-muted-foreground">Sin registros de mantenimiento</td></tr>}
            {logs.map((log: any) => (
