...
function CreateAssetDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const createMutation = trpc.assets.create.useMutation({
    onSuccess: () => { utils.assets.list.invalidate(); toast.success("Activo registrado"); onClose(); },
    onError: (e) => toast.error(e.message),
  });

  const [form, setForm] = useState({
    assetCode: `ACT-${Date.now().toString(36).toUpperCase()}`,
    name: "", category: "camera" as const, brand: "", model: "", serialNumber: "",
    status: "active" as const, criticality: "medium" as const, location: "",
    installDate: "", warrantyExpiry: "", usefulLifeYears: 5,
    purchaseCost: "", currentValue: "", replacementCost: "", maintenanceCostYearly: "",
    depreciationMethod: "straight_line" as const, notes: "",
  });

  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader><DialogTitle className="font-display">Registrar Activo</DialogTitle></DialogHeader>
        <div className="grid grid-cols-2 gap-3 py-2">
          <div className="space-y-1.5">
            <Label className="text-xs">Código de Activo</Label>
            <Input value={form.assetCode} onChange={(e) => setForm({ ...form, assetCode: e.target.value })} className="text-sm" />
          </div>
          <div className="space-y-1.5">
          ...
          <div className="col-span-2 space-y-1.5">
            ...
            <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Ej: Cámara IP Domo Exterior #01" className="text-sm" />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs">Marca</Label>
            <Input value={form.brand} onChange={(e) => setForm({ ...form, brand: e.target.value })} placeholder="Hikvision, Dahua..." className="text-sm" />
          </div>
          <div className="space-y-1.5">
          ...
          <div className="col-span-2 space-y-1.5">
            ...
            <Input value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} placeholder="Ej: Planta baja, entrada principal" className="text-sm" />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs">Fecha de Instalación</Label>
            <Input type="date" value={form.installDate} onChange={(e) => setForm({ ...form, installDate: e.target.value })} className="text-sm" />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs">Vida Útil (años)</Label>
            <Input type="number" value={form.usefulLifeYears} onChange={(e) => setForm({ ...form, usefulLifeYears: Number(e.target.value) })} className="text-sm" />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs">Costo de Adquisición (MXN)</Label>
            <Input type="number" value={form.purchaseCost} onChange={(e) => setForm({ ...form, purchaseCost: e.target.value })} className="text-sm" />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs">Valor Actual (MXN)</Label>
            <Input type="number" value={form.currentValue} onChange={(e) => setForm({ ...form, currentValue: e.target.value })} className="text-sm" />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs">Costo de Reemplazo (MXN)</Label>
            <Input type="number" value={form.replacementCost} onChange={(e) => setForm({ ...form, replacementCost: e.target.value })} className="text-sm" />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs">Costo Mant. Anual (MXN)</Label>
            <Input type="number" value={form.maintenanceCostYearly} onChange={(e) => setForm({ ...form, maintenanceCostYearly: e.target.value })} className="text-sm" />
          </div>
        </div>
        <DialogFooter>
