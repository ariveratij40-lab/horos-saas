  ...
  const rows = await tx<EvidenceRow[]>`
    ...
    JOIN LATERAL (
      ...
      WHERE e.service_request_id = r.id
        ...
        AND e.metadata ? 'policyId'
        AND e.metadata ? 'policyServiceId'
...
async function resolveCanonicalCoverage(
  tx: TransactionSql,
  ticketId: string,
  priority: z.infer<typeof prioritySchema>,
): Promise<CanonicalCoverage | null> {
  const evidence = await loadEvidence(tx, ticketId);
  if (!evidence) return null;

  const metadata = asRecord(evidence.metadata);
  const policyId = metadata ? asString(metadata.policyId) : null;
  const policyServiceId = metadata
    ? asString(metadata.policyServiceId)
    : null;

  if (!policyId || !policyServiceId) return null;
  ...
  const rows = await tx<{
    ...
    WHERE p.id = ${policyId}::uuid
      AND ps.id = ${policyServiceId}::uuid
...
export const serviceRequestSlaRecoveryRobustRouter = router({
      ...
      .query(async ({ ctx, input }) => {
        return withTenantTransaction(
          ...
          async tx => {
            ...
            const coverage = await resolveCanonicalCoverage(
              tx,
              ticket.id,
              ticket.priority,
            );
            ...
            if (!coverage) {
              return {
                recoverable: false,
                reason: "no_canonical_policy_reference" as const,
                ticketNumber: ticket.ticketNumber,
              };
            }
          ...
          async tx => {
            ...
            const snapshots = await tx<{ id: string }[]>`
              ...
              VALUES (
                ...
                ${actorName(ctx)},
                ${JSON.stringify({
                  action: "sla_recovered_from_service_request_reference",
