 ...
 *
 * Every state transition and its audit event are written in the same tenant
 * transaction. Administrative review actions remain separate from requester
 * response actions so a request cannot skip the information-response loop.
 */
export const serviceRequestWorkflowRouter = router({
  canonicalSubmit:
    pgProtectedProcedure
  ...
  canonicalRequestInformation:
    pgProtectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        requireReviewerRole(ctx.pgTenant.tenantRole);
          ...
          async tx => {
            ...
            const rows = await tx<{
              ...
              SET
                ...
                clarity_summary = ${input.message ?? null},
                missing_information =
                  ${JSON.stringify(input.missingInformation)}::jsonb,
            ...
            await tx`
              ...
              VALUES (
                ...
                ${actorName(ctx.user)},
                ${input.message ?? "Additional information requested"},
                ${JSON.stringify({
                  fromStatus,
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        return withTenantTransaction(
          ...
          async tx => {
            ...
            // A request in needs_information must first be answered through
            // requester.canonicalProvideInformation, which returns it to
            // submitted and records requester_confirmed_at atomically.
            ...
            const pendingItems = current[0]!.missingInformation;
            if (Array.isArray(pendingItems) && pendingItems.length > 0) {
              throw new TRPCError({
                code: "CONFLICT",
                message:
                  "Pending requester information must be resolved before review",
              });
            }
            ...
            if (rows.length !== 1) {
              throw new TRPCError({
                code: "CONFLICT",
                message:
                  "Service request changed before review readiness could be saved",
              });
            }
        ...
        return withTenantTransaction(
          ...
          async tx => {
            ...
            return tx<{
              ...
              WHERE service_request_id = ${input.id}::uuid
